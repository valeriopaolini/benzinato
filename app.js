'use strict';
const APP_VERSION='1.0.0', SETTINGS_KEY='benza.settings.v1', SPLASH_KEY='benza.splashDate';
...
let settings={...defaults,...readSettings()},manifest=null,localData=[],map=null,markers=null,pendingFocus=null,availableUpdate=false;
...
function openDb(){return new Promise((resolve,reject)=>{const req=indexedDB.open('benza-data',1);req.onupgradeneeded=()=>req.result.createObjectStore('datasets',{keyPath:'key'});req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
...
function validateManifest(value){if(value?.schemaVersion!==1||!Array.isArray(value.provinces))throw new Error('Manifest non valido')}
function manifestFuels(){const result=new Map();for(const p of manifest?.provinces||[])for(const f of p.fuels||[])if(!result.has(f.id))result.set(f.id,f.label);return result}
function requiredFiles(){if(!manifest)return[];return manifest.provinces.filter(p=>settings.provinces.includes(p.code)).flatMap(p=>p.fuels.filter(f=>!settings.fuel||f.id===settings.fuel).map(f=>({...f,province:p.code,key:`${p.code}/${f.id}`})))}
...
async function downloadData(){
  ...
  try{for(let i=0;i<needed.length;i++){const file=needed[i],response=await fetch(file.path,{cache:'no-cache'});if(!response.ok)throw new Error(file.path);const records=await response.json();if(!Array.isArray(records))throw new Error('Dataset non valido');await dbPut({key:file.key,province:file.province,fuel:file.id,sha256:file.sha256,generatedAt:manifest.generatedAt,records,bytes:file.bytes});button.textContent=`Download ${i+1}/${needed.length}`}
...
function renderFuelOptions(){const fuels=manifestFuels();const options=[...fuels].map(([id,label])=>`<option value="${esc(id)}">${esc(label)}</option>`).join('');for(const id of ['fuel','quick-fuel']){$(id).innerHTML=options;if(fuels.has(settings.fuel))$(id).value=settings.fuel}if(!settings.fuel&&fuels.size){settings.fuel=fuels.keys().next().value;saveSettings();$('fuel').value=settings.fuel;$('quick-fuel').value=settings.fuel}}
function syncQuick(){$('quick-mode').value=settings.mode;$('quick-radius').value=settings.radius;$('quick-sort').value=settings.sort;if(settings.fuel)$('quick-fuel').value=settings.fuel}
...
function matchingRecords(withRadius=true){const origin=currentLocation(),records=localData.filter(d=>d.fuel===settings.fuel).flatMap(d=>d.records).filter(r=>r.isSelf===(settings.mode==='self')).map(r=>({...r,distance:origin?distanceKm(origin[0],origin[1],r.latitude,r.longitude):null}));const filtered=withRadius&&origin?records.filter(r=>r.distance<=settings.radius):records;return filtered.sort(settings.sort==='distance'?(a,b)=>(a.distance??Infinity)-(b.distance??Infinity)||a.price-b.price:(a,b)=>a.price-b.price||(a.distance??Infinity)-(b.distance??Infinity))}
function renderRanking(){
  const list=$('ranking-list'),label=manifestFuels().get(settings.fuel)||'Carburante';$('ranking-summary').textContent=`${label} · ${settings.mode==='self'?'Self-service':'Servito'} · ${settings.radius} km`;
  ...
  list.innerHTML=records.map(r=>{const age=ageInfo(r.reportedAt);return`<article class="station" data-id="${r.id}" data-lat="${r.latitude}" data-lon="${r.longitude}"><div class="price ${age.state}">€ ${r.price.toFixed(3)}<small>${age.text}</small></div><div><h3>${esc(r.name)}${r.brand?` · ${esc(r.brand)}`:''}</h3><p>${r.distance.toFixed(r.distance<10?1:0)} km · ${r.isSelf?'Self-service':'Servito'}</p><p>${esc(r.address)} · ${esc(r.municipality)}</p></div><button class="nav-out" data-nav="${r.id}" aria-label="Apri navigatore">➜</button></article>`}).join('');
  list.querySelectorAll('.station').forEach(el=>el.addEventListener('click',()=>{const r=records.find(x=>String(x.id)===el.dataset.id);pendingFocus=r;showView('map')}));list.querySelectorAll('[data-nav]').forEach(el=>el.addEventListener('click',event=>{event.stopPropagation();openNavigation(records.find(x=>String(x.id)===el.dataset.nav))}))
}
...
function renderMap(){initMap();setTimeout(()=>{map?.invalidateSize();const all=matchingRecords(false);if(pendingFocus){map.setView([pendingFocus.latitude,pendingFocus.longitude],16);pendingFocus=null}else if(currentLocation()&&map.getZoom()<7)map.setView(currentLocation(),11);renderMapMarkers();$('map-empty').hidden=all.length>0;$('map-empty').textContent=all.length?'':'Nessun distributore nei dati caricati. La mappa non scarica altre province.'},50)}
function renderMapMarkers(){if(!map||!markers)return;markers.clearLayers();const bounds=map.getBounds();for(const r of matchingRecords(false).filter(r=>bounds.contains([r.latitude,r.longitude]))){const age=ageInfo(r.reportedAt),icon=L.divIcon({className:'price-marker',html:`<div class="${age.state}" aria-label="€ ${r.price.toFixed(3)}, ${age.text}">€${r.price.toFixed(3)}</div>`,iconSize:[64,30],iconAnchor:[32,15]});L.marker([r.latitude,r.longitude],{icon,title:`${r.name}, € ${r.price.toFixed(3)}`}).on('click',()=>openNavigation(r)).addTo(markers)}}
...
function bind(){
  ...
  ['quick-fuel','quick-mode','quick-radius','quick-sort'].forEach(id=>$(id).addEventListener('change',()=>{settings[{ 'quick-fuel':'fuel','quick-mode':'mode','quick-radius':'radius','quick-sort':'sort'}[id]]=$(id).type==='number'?Number($(id).value):$(id).value;saveSettings();syncForm();renderCurrent()}));
