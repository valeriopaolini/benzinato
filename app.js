...
function renderMapDetail(){const el=$('map-detail');if(!selectedStation){el.hidden=true;el.innerHTML='';return}const r=selectedStation,age=ageInfo(r.reportedAt),distance=r.distance==null?'':`<span>${r.distance.toFixed(r.distance<10?1:0)} km</span>`;el.innerHTML=`<div class="map-detail-copy"><h2>${esc(r.name)}${r.brand?` · ${esc(r.brand)}`:''}</h2><p>${distance}<span>${r.isSelf?'Self-service':'Servito'}</span><span class="${age.state}">€ ${r.price.toFixed(3)} · ${age.text}</span></p><p>${esc(r.address)} · ${esc(r.municipality)}</p></div><button id="map-navigate" class="nav-out" aria-label="Apri navigatore">➜</button>`;el.hidden=false;$('map-navigate').onclick=()=>openNavigation(r)}
